									 ...
									 SNDAECSResult* ret)
{
	...
	char host[S_SNDA_ECS_MAX_HOST_LEN];
	if(!snda_ecs_get_region_host(region,host)) {
	   snda_ecs_set_handler_error(ret, "Invalid Region(region currently support huadong-1 and huabei-1).");
...
{
    const char* snda_ecs_region_default = "huadong-1";
    xmlDocPtr doc = 0;
    xmlNodePtr cur  = 0;
		...
		const char* secretkey, const char* bucketname, int ssl, SNDAECSResult* ret)
{
	...
	return snda_ecs_bucket_opt(handler, accesskey, secretkey,
			bucketname, region, subresource, subresource, ssl, 0,
			SNDA_ECS_DELETE, SNDA_ECS_FOLLOW_LOCATION, 1,
			ret);
}
