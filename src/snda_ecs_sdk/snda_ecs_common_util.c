...
static const SNDAECSPair s_snda_ecs_region_host_map[] = {
	{"huabei-1","storage-huabei-1.grandcloud.cn"},
	{"huadong-1","storage-huadong-1.grandcloud.cn"}
};
...
char* snda_ecs_get_region_host(const char * region,char * region_host) {
	...
	if(region) {
	   SNDAECSPair* match = (SNDAECSPair*)bsearch(region,
				s_snda_ecs_region_host_map,
				sizeof(s_snda_ecs_region_host_map)/sizeof(SNDAECSPair),
				sizeof(SNDAECSPair),
				snda_ecs_search_comparator);
		if (match) {
			sprintf(region_host, "%s", match->value);
		} else {
		    return NULL;
		}
	} else {
	        sprintf(region_host,"%s",snda_ecs_default_hostname);
	}
	return region_host;
}

char* snda_ecs_get_content_type(const char* filename, char* contenttype) {
	const char* snda_ecs_default_content_type = "binary/octet-stream";
...
SNDAECSCommonPrefix* snda_ecs_to_common_prefix(const xmlNodePtr cpelementnode)
{
	...
	node = snda_ecs_init_common_prefix();
	value = (char*)(cpelementnode->xmlChildrenNode->children->content);
