...
SNDAECSHandler* snda_ecs_init_handler() {
	...
	char* handlererrmsg = (char *)calloc(CURL_ERROR_SIZE, sizeof(char));
	if (!handlererrmsg) {
		curl_easy_cleanup(curl);
		return NULL;
	}

	SNDAECSHandler * handler = malloc(sizeof(SNDAECSHandler));
	if (!handler) {
		curl_easy_cleanup(curl);
		free(handlererrmsg);
		return NULL;
...
SNDAECSErrorResponseContent* snda_ecs_to_error_response(SNDAECSResult* result) {
	...
	xmlDocPtr doc = xmlParseMemory(result->serverresponse->responsebody->retbody + i,
	...
	SNDAECSErrorResponseContent* content = snda_ecs_init_error_response_content();
